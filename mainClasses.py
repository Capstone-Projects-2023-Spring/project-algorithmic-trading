...
# To use Pydoc, use command from terminal:
# python -m pydoc -w <Directory to file>\filename
# Don't include .py in filename
...
class DatabaseThread(Thread):
    ...
    def __init__(self, host, db_name, db_user, db_password):
        ...
        Args:
            ...
            db_name (str): The name of the database on the host that we are forming a connection to
            db_user (str): The username credential necessary for
            db_password (str): The password to use for the database connection.

        Other Class Variables:
            connection (psycopg2.connection): An object that tests and creates connection to database with information
            previously specified in the __init__ arguments.
            cursor (psycopg2.connection.cursor): A cursor created from the psycopg2 connection that allows
            queries to be run, and their returns to be captured
        """
        super().__init__()
        self.host = host
